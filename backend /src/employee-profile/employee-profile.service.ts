...
export class EmployeeProfileService {
  ...
  async deactivate(id: string) {
    ...
    const updated = await this.employeeModel
      .findByIdAndUpdate(id, { status: EmployeeStatus.INACTIVE }, { new: true })
      .lean();
  ...
  async selfUpdate(employeeId: string, dto: SelfUpdateDto) {
    ...
    const updated = await this.employeeModel
      .findByIdAndUpdate(employeeId, payload, { new: true })
      .lean();
  ...
  async approveChangeRequest(changeRequestMongoId: string) {
    const request =
      await this.changeRequestModel.findById(changeRequestMongoId);
    if (!request) {
  ...
  async getTeamSummaryForManager(managerId: string) {
    return this.employeeModel
      ...
      .select(
        'firstName lastName primaryDepartmentId primaryPositionId employeeStatus',
      )
      .populate('primaryDepartmentId')
      .populate('primaryPositionId')
      .lean();
