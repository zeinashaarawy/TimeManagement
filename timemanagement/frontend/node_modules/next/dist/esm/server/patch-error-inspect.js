 ...
 */ function getSourcemappedFrameIfPossible(frame, sourceMapCache, inspectOptions) {
    ...
    const applicableSourceMap = findApplicableSourceMapPayload((frame.line1 ?? 1) - 1, (frame.column1 ?? 1) - 1, sourceMapPayload);
...
function parseAndSourceMap(error, inspectOptions) {
    ...
    for(let i = 0; i < sourceMappedFrames.length; i++){
        ...
        if (!frame.stack.ignored) {
            ...
            sourceMappedStack += '\n' + dim(frameToString(frame.stack.methodName, frame.stack.file, frame.stack.line1, frame.stack.column1));
        }
    }
