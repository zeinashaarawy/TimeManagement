 ...
 */ export default function LinkComponent(props) {
    ...
    const observeLinkVisibilityOnMount = React.useCallback((element)=>{
        if (router !== null) {
            linkInstanceRef.current = mountLinkInstance(element, href, router, fetchStrategy, prefetchEnabled, setOptimisticLinkStatus);
        }
        return ()=>{
...
function getFetchStrategyFromPrefetchProp(prefetchProp) {
    if (process.env.__NEXT_CACHE_COMPONENTS && process.env.__NEXT_CLIENT_SEGMENT_CACHE) {
        ...
        return prefetchProp === null || prefetchProp === 'auto' ? // If we're not using `clientSegmentCache`, this will be converted into a `PrefetchKind.AUTO`.
        FetchStrategy.PPR : // To preserve backwards-compatibility, anything other than `false`, `null`, or `"auto"` results in a full prefetch.
