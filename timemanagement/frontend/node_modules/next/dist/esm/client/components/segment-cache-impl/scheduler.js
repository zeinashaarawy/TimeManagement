                    ...
                    case FetchStrategy.LoadingBoundary:
                        {
                            ...
                            let needsDynamicRequest = spawnedEntries.size > 0;
                            if (!needsDynamicRequest && route.isHeadPartial && route.TODO_metadataStatus === EntryStatus.Empty) {
...
function pingSharedPartOfCacheComponentsTree(now, task, route, oldTree, newTree) {
    ...
    const segment = readOrCreateSegmentCacheEntry(now, task.fetchStrategy, route, newTree.cacheKey);
...
function pingNewPartOfCacheComponentsTree(now, task, route, tree) {
    ...
    const segment = readOrCreateSegmentCacheEntry(now, task.fetchStrategy, route, tree.cacheKey);
        ...
        case EntryStatus.Empty:
            {
                ...
                spawnedEntries.set(tree.cacheKey, upgradeToPendingSegment(segment, // Set the fetch strategy to LoadingBoundary to indicate that the server
...
function pingPPRSegmentRevalidation(now, route, routeKey, tree) {
    const revalidatingSegment = readOrCreateRevalidatingSegmentEntry(now, FetchStrategy.PPR, route, tree.cacheKey);
    switch(revalidatingSegment.status){
        case EntryStatus.Empty:
            // Spawn a prefetch request and upsert the segment into the cache
            // upon completion.
            upsertSegmentOnCompletion(spawnPrefetchSubtask(fetchSegmentOnCacheMiss(route, upgradeToPendingSegment(revalidatingSegment, FetchStrategy.PPR), routeKey, tree)), getGenericSegmentKeypathFromFetchStrategy(FetchStrategy.PPR, route, tree.cacheKey));
            break;
        case EntryStatus.Pending:
            break;
        case EntryStatus.Fulfilled:
...
function pingFullSegmentRevalidation(now, route, tree, fetchStrategy) {
    const revalidatingSegment = readOrCreateRevalidatingSegmentEntry(now, fetchStrategy, route, tree.cacheKey);
    if (revalidatingSegment.status === EntryStatus.Empty) {
        ...
        upsertSegmentOnCompletion(waitForSegmentCacheEntry(pendingSegment), getGenericSegmentKeypathFromFetchStrategy(fetchStrategy, route, tree.cacheKey));
        ...
        if (canNewFetchStrategyProvideMoreContent(nonEmptyRevalidatingSegment.fetchStrategy, fetchStrategy)) {
            ...
            const emptySegment = overwriteRevalidatingSegmentCacheEntry(fetchStrategy, route, tree.cacheKey);
