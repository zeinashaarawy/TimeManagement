...
export async function fetchSegmentPrefetchesUsingDynamicRequest(task, route, fetchStrategy, dynamicRequestTree, spawnedEntries) {
    ...
    try {
        ...
        const serverData = await createFromNextReadableStream(prefetchStream, headers);
        const isResponsePartial = fetchStrategy === FetchStrategy.PPRRuntime ? !!response.headers.get(NEXT_DID_POSTPONE_HEADER) : // (even if we did set the prefetch header, we only use this codepath for non-PPR-enabled routes)
        false;
...
function writeDynamicTreeResponseIntoCache(now, task, fetchStrategy, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled) {
    ...
    const staleTimeMs = staleTimeHeaderSeconds !== null ? getStaleTimeMs(parseInt(staleTimeHeaderSeconds, 10)) : STATIC_STALETIME_MS;
    ...
    for (const flightData of flightDatas){
        ...
        if (seedData !== null) {
            ...
            for(let i = 0; i < segmentPath.length; i += 2){
                ...
                requestKey = appendSegmentRequestKeyPart(requestKey, parallelRouteKey, requestKeyPart);
                cacheKey = appendSegmentCacheKeyPart(cacheKey, parallelRouteKey, createSegmentCacheKeyPart(requestKeyPart, segment));
            }
