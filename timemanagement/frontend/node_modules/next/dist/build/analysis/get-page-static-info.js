...
async function getAppPageStaticInfo({ pageFilePath, nextConfig, isDev, page }) {
    var // don't allow in `clientSegmentCache: 'client-only'` mode
    ...
    if ('unstable_prefetch' in config && (!nextConfig.cacheComponents || ((_nextConfig_experimental = nextConfig.experimental) == null ? void 0 : _nextConfig_experimental.clientSegmentCache) !== true)) {
        throw Object.defineProperty(new Error(`Page "${page}" cannot use \`export const unstable_prefetch = ...\` without enabling \`cacheComponents\` and \`experimental.clientSegmentCache\`.`), "__NEXT_ERROR_CODE", {
            value: "E885",
            enumerable: false,
            configurable: true
        });
